...
public class PuttingSimulator{
	...
	public void take_shot(Vector2d initial_ball_velocity){
		...
		while(Tools.advRound(engine.get_v().get_x(), 6) != 0 ||Tools.advRound(engine.get_v().get_y(), 6) != 0) {
			engine.actTimestep();
		...
		System.out.println(">> Shot landed at ( " + ball_position.get_x() + " , " + ball_position.get_y() + " ).");
	}

	public boolean calcWin(Vector2d _ball_position){ // I added  a parameter in the method, could not get it to work without it!!!!!!
		boolean r = false;
		double absDis = Math.sqrt(Math.pow(_ball_position.get_x() - course.get_flag_position().get_x(),2) + Math.pow(_ball_position.get_y() - course.get_flag_position().get_y(),2));
